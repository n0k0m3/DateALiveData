...
function AddressConfirmView:ctor(...)
    ...
    self:initData(...)
    self:init("lua.uiconfig.activity.addressConfirmView")
end
...
function AddressConfirmView:showBefore()
    local beforeInfo = ActivityDataMgr2:getFillInfo()
...
function AddressConfirmView:showAfter()
    ...
    if beforeInfo.email ~= email then
        self.afterInfo_.email:setColor(me.RED)
    else
    ...
    if beforeInfo.phone ~= phone then
        self.afterInfo_.phone:setColor(me.RED)
    else
    ...
    if beforeInfo.qq ~= qq then
        self.afterInfo_.qq:setColor(me.RED)
    else
    ...
    if self.newFillInfo.selectInfo then
        ...
        if oldStr ~= selectStr then
            self.afterInfo_.address:setColor(me.RED)
        else
...
function AddressConfirmView:onUpdateChangeAddress()
    ...
    ActivityDataMgr2:setFillInfo(self.newFillInfo)
    AlertManager:closeLayer(self)
end

function AddressConfirmView:registerEvents()

    EventMgr:addEventListener(self,EV_UPDATE_CHANGE_ASSIST_ADDRESS,handler(self.onUpdateChangeAddress, self))
    ...
    self.Button_submit:onClick(function()
        ...
        ActivityDataMgr2:Send_submitAddress(jsonData)
    end)
end
