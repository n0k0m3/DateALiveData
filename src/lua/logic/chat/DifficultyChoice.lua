...
function DifficultyChoice:initUI(ui)
    ...
    dump(selects)
    for i=1,5 do
        local node = TFDirector:getChildByPath(self.Panel_root,"Button_"..tostring(i)) 
        ...
        if i == 5 then
            local maxCount = TabDataMgr:getData("DiscreteData",81012).data.weekTime[5]
