...
function BaoshiComposeView:initData()
    self.rarity_ = self.paramData_.rarity or 2
    self.heroId_ = 0
    self.selectPos = 0
...
function BaoshiComposeView:initUI(ui)
    ...
    self.listView_tuzhi = UIListView:create(ScrollView_tuzhi)
    ...
    for i = 1, 5 do
        local foo = {}
        foo.root = TFDirector:getChildByPath(self.Panel_pos,"Panel_pos_"..i):hide()
    ...
    self.spineLineAnim = {}
    for i=1,5 do
        local Spine_line = TFDirector:getChildByPath(self.Panel_pos,"Spine_line"..i):hide()
        self.spineLineAnim[i] = Spine_line
    end
    self.spineEffectAnim = {}
    for i=1,5 do
        local Spine_effect = TFDirector:getChildByPath(self.Panel_pos,"Spine_effect"..i):hide()
        self.spineEffectAnim[i] = Spine_effect
    end

    self.Panel_get_gem = TFDirector:getChildByPath(self.Panel_pos,"Panel_get_gem"):hide()
    self.Spine_compose = TFDirector:getChildByPath(self.Panel_pos,"Spine_compose"):hide()
    self.Image_bg = TFDirector:getChildByPath(self.Panel_get_gem,"Image_bg")
    self.Image_quality_bg = TFDirector:getChildByPath(self.Panel_get_gem,"Image_quality_bg")
    self.Image_quality = TFDirector:getChildByPath(self.Panel_get_gem,"Image_quality")
    self.Image_icon = TFDirector:getChildByPath(self.Panel_get_gem,"Image_icon")

    self.Image_pos_bg = TFDirector:getChildByPath(self.Panel_pos,"Image_pos_bg")
    self.Image_tuzhi_icon = TFDirector:getChildByPath(self.Panel_pos,"Image_tuzhi_icon")
...
function BaoshiComposeView:updateTuzhiUI()
    ...
    self.selectData = {}
    self.selectTuzhiIdx = nil
    self.listView_tuzhi:removeAllItems()
    ...
    for i, v in ipairs(self.tuzhiData) do
        local item = self.Panel_tuzhi_item:clone()
        self:updateTuzhiItem(item, v)
        self.listView_tuzhi:pushBackCustomItem(item)
        item:setTouchEnabled(true)
        item:onClick(function()
            self:selectTuzhi(i, item)
        end)
        if i == 1 then
            self:selectTuzhi(i, item)
        end
...
function BaoshiComposeView:updateTuzhiItem(item, cfg)
    ...
    Label_count:setText(tostring(GoodsDataMgr:getItemCount(cfg.drawingId)))
    local gemCfg = EquipmentDataMgr:getGemCfg(cfg.drawingId)
    Image_bg:setTexture(EC_ItemIcon[gemCfg.quality])
...
function BaoshiComposeView:updateGemPosUI()
    ...
    self.Image_pos_bg:setTexture(EquipmentDataMgr:getGemComposeBg(graph))
    self.Image_tuzhi_icon:setTexture(self.selectTuzhiIdx and tuzhiCfg.icon or self:getDefaultTuzhi())
    ...
    for i,v in ipairs(self.gem_pos) do
        if i <= graph then
            ...
            self:updatePosItem(i, v.root)
        else
            v.root:hide()
        end
    end
    for i = 1, graph do
        self.spineEffectAnim[i]:setVisible(false)
    end
end
...
function BaoshiComposeView:updateSpineLineAnim(success)
    ...
    if count >= graph and success then
        ...
        for i = 1, graph do
            local line = self.spineLineAnim[i]
    ...
    elseif count < graph then
        for i = 1, graph do
            self.spineLineAnim[i]:setVisible(false)
        end
    end
end
...
function BaoshiComposeView:updateButtonState()
    self.button_left:setVisible(self.rarity_ > 2)
    self.button_right:setVisible(self.rarity_ < 6)
    local composeCfg = self.tuzhiData[self.selectTuzhiIdx or 1] or self.composeCfgs[1]
    local graph = composeCfg and composeCfg.graph or 3
    local count = table.count(self.selectData)
...
function BaoshiComposeView:changeRarity(isLeft)
    if isLeft then
...
function BaoshiComposeView:onComposeBack(data)
    ...
    local function comeposeOver()
        ...
        self.Panel_get_gem:setVisible(false)
        if data.gem then
    ...
    self.Panel_touch:onClick(function()
        ...
        for k, effect in pairs(self.spineEffectAnim) do
    ...
    for i = 1, graph do
        local effect = self.spineEffectAnim[i]
    ...
    self:timeOut(function()
        ...
        self.Spine_compose:addMEListener(TFARMATURE_COMPLETE,function()
            ...
            self.Spine_compose:addMEListener(TFARMATURE_COMPLETE,function()
                ...
                self.Spine_compose:setVisible(false)
                self.Panel_get_gem:setVisible(true)
                if data.gem then
                    local cfg = EquipmentDataMgr:getGemCfg(data.gem.cid)
                    self.Image_icon:setTexture(cfg.icon)
                    self.Image_bg:setTexture(EquipmentDataMgr:getGemIconBg(cfg.quality))
                    self.Image_quality_bg:setTexture(EquipmentDataMgr:getGemQualityBg(cfg.quality))
                    self.Image_quality:setTexture(EquipmentDataMgr:getGemRarityIcon(cfg.rarity))
                    self:timeOut(function()
                        comeposeOver()
                    end, 0.4)
                end
