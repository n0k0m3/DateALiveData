local DalMapFight = class("DalMapFight", BaseLayer)

function DalMapFight:ctor(ambushId)
    self.super.ctor(self)
    --self:showPopAnim(true)
    self:initData(ambushId)
    self:init("lua.uiconfig.dls.dalMapFight")
end

function DalMapFight:initData(ambushId)

    self.ambushId = ambushId
    print("self.ambushId",self.ambushId)
...
function DalMapFight:initUI(ui)
    ...
    self.Image_costbg = TFDirector:getChildByPath(self.ui, "Image_costbg"):hide()
...
function DalMapFight:initUiInfo()
...
function DalMapFight:calMoveRect()
    self.moveRect_ = {}
...
function DalMapFight:onUpdateTeamInfo()

    self.formationData_ = {}
    self.forbidHero = {}
    self.formation = DalMapDataMgr:getFormation()
    for i = 1, 3 do
...
function DalMapFight:replaceFormation(fromHeroId, toHeroId)
...
function DalMapFight:registerEvents()

    EventMgr:addEventListener(self,EV_DAL_MAP.Formation,handler(self.onUpdateTeamInfo, self))
    ...
    for i=1,3 do
        self.teamInfo[i].Button_add:onClick(function()
            Utils:openView("dalMap.DalMapFormation",i)
        end)
    end
