local PhoneMainViewAIH = class("PhoneMainViewAIH", BaseLayer)

function PhoneMainViewAIH:ctor(data)
    self.super.ctor(self)
    self:showPopAnim(true)
    self:initData(data)
    self:init("lua.uiconfig.iphone.PhoneMainViewAIH")
end

function PhoneMainViewAIH:initUI(ui)
    ...
    self.Image_wifi = TFDirector:getChildByPath(self.Panel_PhoneMain,"Image_wifi")
    self.winSize = me.Director:getVisibleSize()
    self.ui:setContentSize(CCSizeMake(640,self.winSize.height))
    ...
    self.contactPLsize = 1160--self.winSize.height - self.titleBgSize
    ...
    self.chatListView:setItemsMargin(20)
    ...
    self.TextButton_input = TFDirector:getChildByPath(self.ui,"TextButton_input"):show()
    self.tf_input = TFDirector:getChildByPath(self.ui,"TextField_input")
    self.tf_input.type = "input"
    self.tf_input:hide()
    self.Button_input = TFDirector:getChildByPath(self.ui,"Button_input")
    self.Button_input:hide()
...
function PhoneMainViewAIH:onShow()
    self.super.onShow(self)
end

function PhoneMainViewAIH:initData(data)

    local datingRuleId,datingTimeFrame = data.ruleId,data.timeFrame
    --datingRuleId = 2301004
...
--返回按钮
function PhoneMainViewAIH:backPhonePL()
...
function PhoneMainViewAIH:showDetailUI(typeId,chatType,roleId,outTimeRuleId)
    ...
    self.Label_pageName:setTextById(pageNameId)
...
--联系人列表
function PhoneMainViewAIH:updateGroupInfo()
...
function PhoneMainViewAIH:setGroupItemInfo(item,info,tipRoleId)
...
--初始本地聊天记录
function PhoneMainViewAIH:initChatInfo(roleId,outTimeRuleId)
...
--选项界面
function PhoneMainViewAIH:updateOptionListInfo(jumpInfo)
...
function PhoneMainViewAIH:showDationInfo(datingId)
    ...
    if #jumpInfo == 1 then
        ...
        self:removeCountDownTimer()
        self.chatListView:setContentSize(CCSizeMake(640,self.contactPLsize - 450))
        self.chatListView:jumpToBottom()
...
function PhoneMainViewAIH:removeEvents()
    self:removeCountDownTimer()
end

function PhoneMainViewAIH:onCountDownPer()
...
----聊天历史Item
function PhoneMainViewAIH:updateChatItemInfo(datingInfo)
...
----过期约会的奖励显示
function PhoneMainViewAIH:updateOutTimeAward(awardContent)
...
----更新属性显示
function PhoneMainViewAIH:updateDaingAttr(itemId,itemNum)
...
function PhoneMainViewAIH:updateDaingText(text)
...
----过期约会时间显示
function PhoneMainViewAIH:updateOutTimeText(datingRuleId,outTime)
...
----刷新约会地点
function PhoneMainViewAIH:updateDatingPlace(placeInfo)
...
function PhoneMainViewAIH:updateNormalText(text)
...
----更新约会中的聊天列表
function PhoneMainViewAIH:updateChatInfo(datingInfo,jumpInfo)
...
----机器人对话
function PhoneMainViewAIH:showDialogeAI(roleId)

    local isFirst = DatingPhoneDataMgr:isFirstUseAi()
    if  not isFirst then
        local helloStr = TextDataMgr:getText(901177)
        helloStr = "2"..helloStr
        DatingPhoneDataMgr:insertDialogue(roleId,helloStr)
        DatingPhoneDataMgr:setFirstUseAi()
    end

    ---显示表情
    self:showExpressionPL()
    self:hideFacePL()
    self.Panel_input_view:setVisible(true)
...
function PhoneMainViewAIH:showExpressionPL()
...
----AI语音
function PhoneMainViewAIH:updateVoice(isSelf,fileName,isNew,isPlayed)
...
----AI对话
function PhoneMainViewAIH:updateDialogue(isSelf,dialogueStr,isNew)
    ...
    local Image_msg = TFDirector:getChildByPath(cell,"Image_msg")
    local rightText = TFRichText:create(ccs(460, 0))
    ...
    cell:setContentSize(CCSizeMake(606,newHight+24))
    self.chatListView:pushBackCustomItem(cell)
    self.chatListView:jumpToBottom()
...
--语音聊天
function PhoneMainViewAIH:updateRecVoice(emotionState,msg)
    --emotionState:表情ID
...
function PhoneMainViewAIH:onCloseInputLayer()

    if self.inputTextField and self.inputTextField.type == "input" then
...
function PhoneMainViewAIH:onTouchSendBtn()
    self.tf_input:hide()
    self.Button_input:hide()
    self.TextButton_input:show()
...
function PhoneMainViewAIH:insertFace(faceStr)
...
function PhoneMainViewAIH:setInputText(inputText)

    if not self.richInput then
        self.richInput = TFRichText:create(ccs(460, 0))
        self.richInput:setPosition(ccp(-200,0))
        self.richInput:AnchorPoint(me.p(0, 0.5))
        self.Button_input:addChild(self.richInput)
    end

    if not self.testRichInput then
        self.testRichInput = TFRichText:create(ccs(460, 0))
        self.testRichInput:setPosition(ccp(-200,22))
        self.testRichInput:AnchorPoint(me.p(0, 1))
        self.Button_input:addChild(self.testRichInput)
        self.testRichInput:setVisible(false)
    end

    local beforText = inputText

    local faceCfg = DatingPhoneDataMgr:getFaceEmotion()
    for k,v in ipairs(faceCfg) do
        beforText = string.gsub(beforText,v.conversioncode,v.convermotion)
    end
    self.testRichInput:Text(TextDataMgr:getFormatText(TextDataMgr:getTextAttr("r301005"),beforText))

    local sizeW = self.testRichInput:getSize().width
    if sizeW > 410 then
        return
    end
...
--发送按钮cd
function PhoneMainViewAIH:sendBtnCd()
...
--显示live2d界面
function PhoneMainViewAIH:showLive2dView()
...
function PhoneMainViewAIH:createLive2d()
...
function PhoneMainViewAIH:updateLive2dAction(emotionState)
...
function PhoneMainViewAIH:continueAction()
...
function PhoneMainViewAIH:showFacePL()
    self.isShowFacePL = true
    self.Image_face_bg:setVisible(true)
    self.Panel_input_view:setPositionY(-204)
    self.Button_input:setPositionY(-43)
    self.TextButton_input:setPositionY(-43)
    self.chatListView:setContentSize(CCSizeMake(640, self.contactPLsize - 448))
    self.chatListView:jumpToBottom()

end

function PhoneMainViewAIH:hideFacePL()
    self.isShowFacePL = false
    self.Image_face_bg:setVisible(false)
    self.Panel_input_view:setPositionY(-589)
    self.Button_input:setPositionY(-43)
    self.TextButton_input:setPositionY(-43)
    self.chatListView:setContentSize(CCSizeMake(640, self.contactPLsize - 120))
    self.chatListView:jumpToBottom()
end

---------------------------------------------反馈消息--------------------------------
function PhoneMainViewAIH:afterGetScript()
    self:showDetailUI(2,"person",self.phoneRole)
end

----完成约会
function PhoneMainViewAIH:onCompletePhoneDating(accept)
...
----
function PhoneMainViewAIH:onShowPhoneAward(flag,ruleId,award)
...
----发送消息的确认
function PhoneMainViewAIH:onSendDialogue(data)
...
----返回机器人对话
function PhoneMainViewAIH:onRecvRobotDialogue(data)
...
----语音SDK反馈
function PhoneMainViewAIH:onRecvVoiceSdk(msg)
    dump(msg)
...
function PhoneMainViewAIH:transformDigueStr(inputStr)
...
function PhoneMainViewAIH:registerEvents()
    ...
    self.Button_Live2Dback:onClick(function()
        ...
        --DatingPhoneDataMgr:sendAiDialogue(EC_PhoneChatType.Voice, "莫西莫西", self.dialogeRoleId)
    end)
...
function PhoneMainViewAIH:onHide()
    self.super.onHide(self)
    Utils:setScreenOrientation(SCREEN_ORIENTATION_LANDSCAPE)
end

function PhoneMainViewAIH:onClose()
    self.super.onClose(self)
    Utils:setScreenOrientation(SCREEN_ORIENTATION_LANDSCAPE)
end

function PhoneMainViewAIH:onExit()
    self.super.onExit(self)
    DatingPhoneDataMgr:writeDialogue()
end

function PhoneMainViewAIH:close()
    --Utils:setScreenOrientation(SCREEN_ORIENTATION_LANDSCAPE)
    --AlertManager:changeScene(SceneType.MainScene)
    AlertManager:closeLayer(self)
end

return PhoneMainViewAIH
