...
-- TextIndex.BAN                                           					   = 3359     --您已被禁止登录
	...
	if data.errorCode == 17429 then
		...
		toastMessageLink(localizable.Zone_somebody_attacking)
...
function ErrorCodeManager:reportErrorMsg(errorMsg)
	...
	if not MainPlayer:getIsEnterGame() then
		return
	end
 ...
 --    UserCenterHttpClient:httpRequest(TFHTTP_TYPE_GET,"http://120.92.44.172:8080/login/account/recordClientErr?err="..errorMsg)
 --    reportErrorTime = nil
