...
local DfwTaskView = class("DfwTaskView", BaseLayer)

function DfwTaskView:initData()
    local store = StoreDataMgr:getStore(EC_StoreType.DAFUWENG)
...
function DfwTaskView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.dafuwong.dfwTaskView")
end

function DfwTaskView:initUI(ui)
...
function DfwTaskView:updateAllTask()
    local commodityData = StoreDataMgr:getCommodity(self.storeCid_)
        print(commodityData ,self.storeCid_)
    ...
    for i, foo in ipairs(self.taskItems_) do
        ...
        local goodsCid, goodsNum = next(commodityCfg.goods)
        PrefabDataMgr:setInfo(foo.Panel_reward, goodsCid, goodsNum)
...
function DfwTaskView:updateRefreshInfo()
    local isAutoRefresh = #self.storeCfg_.autoRefreshCorn > 0
...
function DfwTaskView:refreshView()
    self:initTask()
    self:updateRefreshInfo()
end

function DfwTaskView:registerEvents()
...
function DfwTaskView:onBuyInfoUpdateEvent()
end

function DfwTaskView:onRefreshEvent(storeId)
    self:updateAllTask()
    self:updateRefreshInfo()
end

function DfwTaskView:onItemUpdateEvent()

end

function DfwTaskView:onBuySuccessEvent(data)
    self:updateAllTask()
