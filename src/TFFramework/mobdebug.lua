...
local function checkFileName(fileName)
  ...
  fileName = fileName:gsub("\\\\", "\\")
  local gg = fileName:split("\\")
  while true do 
    local nLen = #gg
    local canDO = false
    for idx, name in ipairs(gg) do 
      if name == ".." then 
        table.remove(gg, idx - 1)
        table.remove(gg, idx - 1)
        canDO = true
        break
      end
...
local function set_breakpoint(file, line)
  ...
  if not filePathCache[file] then 
    local oldFile = file
    while true do
      local tempFile = me.FileUtils:fullPathForFilename(file)
      if iscasepreserving then 
        tempFile = string.lower(tempFile)
      end
      if tempFile == file then 
        if file[1] == '/' or file[1] == '\\' then 
          file = file[2]
        else 
          break
        end
    ...
    filePathCache[oldFile] = file
  else 
    file = filePathCache[file]
  end
...
local function remove_breakpoint(file, line)
  ...
  if not filePathCache[file] then 
    ...
    tempFile = checkFileName(tempFile)
    filePathCache[file] = tempFile
    file = filePathCache[file]
  else 
    file = filePathCache[file]
  end
