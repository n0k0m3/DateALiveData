...
function EquipSuitUpView:onStrengthenOver(data)
    if data.isSuccess then
        ...
        self.Spine_atk:show()
        self.Spine_def:show()
        self.Spine_up_success:play("chuxian")
        self.Spine_hp:playByIndex(0, -1, -1, 1)
        self.Spine_atk:playByIndex(0, -1, -1, 1)
        self.Spine_def:playByIndex(0, -1, -1, 1)

        self.Spine_up_success:addMEListener(TFARMATURE_COMPLETE,function()
            self.Spine_up_success:play("xunhuan")
            self.Spine_up_success:removeMEListener(TFARMATURE_COMPLETE)
        end)
