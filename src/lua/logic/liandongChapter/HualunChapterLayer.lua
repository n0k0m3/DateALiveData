...
local HualunChapterLayer = class("HualunChapterLayer",BaseLayer)

function HualunChapterLayer:ctor(groupId, goodId)
	-- body
	self.super.ctor(self)
	self.goodId = goodId
	self.groupId = groupId
	self.levelIds = FubenDataMgr:getLevel(groupId,1)
	self:init("lua.uiconfig.activity.hualunChapterLayer")
end

function HualunChapterLayer:initUI( ui )
	-- body
...
function HualunChapterLayer:registerEvents( ... )
	-- body
	...
	self.Button_help2:onClick(function ( ... )
		-- body
		Utils:showTips(16000484)
	end)
end

function HualunChapterLayer:updateLevelList( )
	-- body
...
function HualunChapterLayer:updateLevelItem(item,levelId)
	-- body
...
function HualunChapterLayer:getNewLevelId( ... )
	-- body
	for k,v in ipairs(self.levelIds) do
...
function HualunChapterLayer:updateContent(  )
	-- body
