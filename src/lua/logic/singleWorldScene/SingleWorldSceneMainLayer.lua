...
local SingleWorldSceneMainLayer = class("SingleWorldSceneMainLayer",BaseLayer)

function SingleWorldSceneMainLayer:ctor( ... )
	-- body
	self.super.ctor(self, ...)
	self:init("lua.uiconfig.activity.noRoker_maplayer")
	self:loadMap()
    WorldRoomDataMgr:getCurControl():setView(self)
end

function SingleWorldSceneMainLayer:initUI(ui)
...
function SingleWorldSceneMainLayer:registerEvents( ... )
	-- body
...
function SingleWorldSceneMainLayer:onRecvChatMsg(chatInfo)
    --显示气泡
    WorldRoomDataMgr:getCurControl():heroTalk(chatInfo)
end

function SingleWorldSceneMainLayer:_onExit( ... )
	-- body
...
--离开大世界
function SingleWorldSceneMainLayer:onLeave()
    AlertManager:closeLayer(self)
end

function SingleWorldSceneMainLayer:onShow()
...
function SingleWorldSceneMainLayer:loadMap()
    ...
    if mainHero then
        ...
        self.rokerView:changeSynicTime(WorldRoomDataMgr:getCurControl():getSynPosTime())
         self.rokerView:setIsSinglePlayer(true)
    end
end

function SingleWorldSceneMainLayer:update(dt)
...
function SingleWorldSceneMainLayer:removeEvents( ... )
...
function SingleWorldSceneMainLayer:hideOrShowUI(uiVisible)
    self.uiPanel:setVisible(uiVisible)
    self.Panel_touch:setVisible(not uiVisible)
end

function SingleWorldSceneMainLayer:updateMiniMap(  )
    -- body
...
function SingleWorldSceneMainLayer:getMotionCfg(  )
    -- body
...
function SingleWorldSceneMainLayer:updateEmojiPanel()
    -- body 
...
function SingleWorldSceneMainLayer:updateEmojiItem( item, data )
    -- body
...
function SingleWorldSceneMainLayer:updateTriggerList( dt )
    -- body
...
function  SingleWorldSceneMainLayer:createActionButtonList( actionList )
    -- body
...
function SingleWorldSceneMainLayer:checkMessageShow(  )
    -- body
...
return SingleWorldSceneMainLayer