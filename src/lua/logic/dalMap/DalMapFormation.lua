local DalMapFormation = class("DalMapFormation", BaseLayer)

function DalMapFormation:ctor(headSlotId)
    self.super.ctor(self)
    self:initData(headSlotId)
    self:showPopAnim(true)
    self:init("lua.uiconfig.dls.dalMapFormation")
end

function DalMapFormation:initData(headSlotId)
...
function DalMapFormation:initUI(ui)
...
function DalMapFormation:initUIInfo()
...
function DalMapFormation:updateHeroList()
...
function DalMapFormation:showSelectHeroInfo()
...
function DalMapFormation:getHandleButtonState()

    --1:移除 2:更换 3：加入 4：已在队伍
    local isHavePlayer = false
    local formation = DalMapDataMgr:getFormation()
    if formation[self.headSlotId] then
        isHavePlayer = true
    end

    local formationSlotId = DalMapDataMgr:beInFormation(self.selectHeroId)
    --所选英雄不在阵型中
...
function DalMapFormation:handleFormation()
    ...
    TFDirector:send(c2s.OFFICE_EXPLORE_OFFICE_FORMATION, msg)
end

function DalMapFormation:registerEvents()
    ...
    EventMgr:addEventListener(self,EV_DAL_MAP.Formation,handler(self.onRecvChangeMember, self))
end

function DalMapFormation:onRecvChangeMember()
    AlertManager:close()
end

return DalMapFormation