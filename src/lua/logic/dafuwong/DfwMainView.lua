...
local DfwMainView = class("DfwMainView", BaseLayer)
...
function DfwMainView:initData()
    local activity = ActivityDataMgr2:getActivityInfoByType(EC_ActivityType2.DAFUWENG)
...
function DfwMainView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.dafuwong.dfwMainView")
end

function DfwMainView:initUI(ui)
    ...
    self.Spine_guang = TFDirector:getChildByPath(self.Button_step, "Spine_guang")
...
function DfwMainView:initRole()
    local rolePath = "modle/citymodle/dafuweng_11103/11103"
    self.Spine_role = SkeletonAnimation:create(rolePath)
    self.Panel_role:addChild(self.Spine_role)
    self.Spine_role:setScale(0.4)
    self.Spine_role:play("dice_stand", true)
end

function DfwMainView:moveRoleToGrid(curSite, targetSite, isJump)
    if isJump then
        ...
        local completeCallfunc = CallFunc:create(function()
                ...
                if cfg.type == 3 then
                    Utils:openView("dafuwong.DfwEventView", self.eventId_, self.eventRewards_, self.eventExRewards_)
                end
                self.isRolling_ = false
...
function DfwMainView:showFloatTip(index, pos)
    ...
    Panel_floatTipItem:AddTo(self.Spine_role):Pos(ccp(0, pos.y + 500))
    Panel_floatTipItem:Alpha(0.3):Scale(2.0)
...
function DfwMainView:moveToGrid(curSite, targetSite)
...
function DfwMainView:updateMap()
...
function DfwMainView:initAllGrid()
...
function DfwMainView:moveMapToGrid(index, isJump, duration)
...
function DfwMainView:refreshView()
...
function DfwMainView:getDiceAnimation(diceCid, point, state)
    local diceCfg = DfwDataMgr:getChessesDiceCfg(diceCid)
    local name = diceCfg.animation .. point .. state
    return name
end

function DfwMainView:updateAllDiceItem()
...
function DfwMainView:updateBuff()
...
function DfwMainView:updateSelectDiceInfo()
...
function DfwMainView:onScrollViewChangedEvent()
...
function DfwMainView:rollDice(diceCid, point)
...
function DfwMainView:registerEvents()
    ...
    self.Button_store:onClick(function()
            Utils:playSound(5004)
            FunctionDataMgr:jStore(308000)
            return true
    end)

    self.Button_turntable:onClick(function()
            Utils:openView("dafuwong.DfwTurntableView")
            Utils:playSound(5004)
            return true
    end)

    self.Button_task:onClick(function()
            Utils:openView("dafuwong.DfwTaskView")
            Utils:playSound(5004)
            return true
    end)

    self.Button_hand_account:onClick(function()
            Utils:openView("dafuwong.DfwHandAccountView")
            Utils:playSound(5004)
            return true
    end)
    ...
    self.Button_step:onClick(function()
            Utils:openView("dafuwong.DfwStepRewardView")
            Utils:playSound(5004)
            return true
    end)
    ...
    self.Button_card:onClick(function()
            Utils:openView("dafuwong.DfwCardView")
            Utils:playSound(5004)
            return true
    end)
...
function DfwMainView:onRollDiceEvent(originLocation, awardStep, buffStep, awardInfo, eventAward)
    self.awardInfo_ = {}
    if awardInfo then
...
function DfwMainView:onUpdateBuffEvent()
    Utils:playSound(5001)
    self.Spine_role:play("dice_cards", false)
    self:updateBuff()
end

function DfwMainView:onUpdateProgressEvent()
    self:updateStepState()
end

function DfwMainView:onItemUpdateEvent()
    self:updateAllDiceItem()
end
