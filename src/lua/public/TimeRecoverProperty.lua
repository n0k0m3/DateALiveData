...
function getTimeString(time)
    ...
    if (time < 0) then
        -- timeStr = "错误时间：" .. time;
        timeStr = TextDataMgr:getText(100000087) .. time;

    elseif (time < 60) then
        -- timeStr = "刚刚";
        timeStr = TextDataMgr:getText(100000088);
    elseif (time < 3600) then
        --分钟
        -- timeStr = math.floor(time/ 60) .. "分钟前";
        timeStr = math.floor(time/ 60) .. TextDataMgr:getText(100000089);
    elseif (time < 24 * 3600) then
        --小时
        -- timeStr = math.floor(time/ 3600) .. "小时前";
        timeStr = math.floor(time/ 3600) .. TextDataMgr:getText(100000090);
    else
        --天
        -- timeStr = math.floor(time/ (3600 * 24)) .. "天前";
        timeStr = math.floor(time/ (3600 * 24)) .. TextDataMgr:getText(100000091);
    end
    return timeStr;
end
...
function TimeRecoverProperty:ctor(maxRecoverTime, curRecoverTime, timeToValuePercent)
    ...
    self.createTime = GetCorrectTime();
end
...
function TimeRecoverProperty:getSubTime()
    return GetCorrectTime() - self.createTime ;
end 
