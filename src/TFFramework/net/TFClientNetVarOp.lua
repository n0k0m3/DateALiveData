...
local InsClientNet = TFClientVarSocket:GetInstance()
local tblUnpackFunc = 0
local tblPackFunc = 0

function netop:setConnectType(eType) 
	InsClientNet:setNetType(eType)
end
...
function netop.UnpackInt64()
	return InsClientNet:UnpackInt64()
end
...
function netop.UnpackTable(tblData , nTagType , nLimitLen)
	...
	while netop.PreUnpackIntVar32() == nTagType do
		...
		if #tblData > 1 or bMulTable == false then-- no repeat msg
			...
			netop.UnpackIntVar32()--single list length
...
function netop.PackInt64(nNum)
	return InsClientNet:PackInt64String(tostring(nNum))
end
...
function netop:SetNetType(eType)
	InsClientNet:setNetType(eType)
end

function netop:GetNetType(eType)
	return InsClientNet:getNetType()
end
...
function netop:SetVarFieldLen(nLen)
	InsClientNet:SetVarFieldLen(nLen);
end

function netop:UnpackDataByRsa(encrypt,encryLen)
	return InsClientNet:UnpackDataByRsa(encrypt,encryLen)
end

function netop:setNewEncryptKey(szKey)
	InsClientNet:setNewEncryptKey(szKey)
end

function netop:getNewEncryptKey( )
	return InsClientNet:getNewEncryptKey()
end

function netop:getRsaDecryptData( )
	return InsClientNet:getRsaDecryptData()
end

function netop.setErrorHandle(nErrorHandle)
	InsClientNet:setErrorHandle(nErrorHandle)
end
netop.__index = netop;
return netop
