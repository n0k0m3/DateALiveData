...
function FileCheckMgr:getCheckUrl()
    ...
    if CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 or RELEASE_TEST then
        checkUrl = urlHead.."DAL_TEST/MainSorceCode/history/version_"..currenVer..".xml";
    elseif CC_TARGET_PLATFORM == CC_PLATFORM_IOS then
        checkUrl = urlHead.."DAL_ANDROID/MainSorceCode/history/version_"..currenVer..".xml";
    elseif CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
        checkUrl = urlHead.."DAL_IOS/MainSorceCode/history/version_"..currenVer..".xml";
    end
    return checkUrl;
end
...
function FileCheckMgr:checkOneFile()
    ...
    for filename,_md5 in pairs(data) do
        ...
        local continue = true
        if string.find(filename,"ui_prj.lua") or string.find(filename,"EditorBase_LoadTFLua") then
            continue = false;
        end
        ...
        if continue then
            ...
            if not string.find(fullpath,"assets/") then
                ...
                if file then
                    ...
                    local _localmd5 = md5.sumhexa(filedata)
                    ...
                    if CC_TARGET_PLATFORM ~= CC_PLATFORM_WIN32 and _localmd5 ~= _md5 then
