...
function StrengthenResult:ctor(data)
    ...
    self:init("lua.uiconfig.fairy.strengthenResult")
end
...
function StrengthenResult:initUI(ui)
	...
	self.Label_title:setTextById(strName[self._type]);
	...
	old_gr_atk:setString(oldGr[EC_Attr.ATK] * lv / 100);
	old_gr_def:setString(oldGr[EC_Attr.DEF] * lv / 100);
	old_gr_hp:setString(oldGr[EC_Attr.HP] * lv / 100);
	...
	cur_gr_atk:setString(curGr[EC_Attr.ATK] * lv / 100);
	cur_gr_def:setString(curGr[EC_Attr.DEF] * lv / 100);
	cur_gr_hp:setString(curGr[EC_Attr.HP] * lv / 100);
