...
local DfwTurntableView = class("DfwTurntableView", BaseLayer)

function DfwTurntableView:initData()
...
function DfwTurntableView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.dafuwong.dfwTurntableView")
end

function DfwTurntableView:initUI(ui)
...
function DfwTurntableView:refreshView()
    ...
    for i, v in ipairs(self.turntable_) do
        ...
        foo.Image_count_normal:setVisible(cfg.rewardMax > 0)
        foo.Image_count_select:setVisible(cfg.rewardMax > 0)
    end
...
function DfwTurntableView:updateTurntableState()
    for i, foo in ipairs(self.turntableItems_) do
        ...
        if foo.Label_count:isVisible() then
            foo.Image_count_select:setVisible(isSelect)
            foo.Image_count_normal:setVisible(not isSelect)
        end
    end
end

function DfwTurntableView:updateTurntableInfo()
    for i, foo in ipairs(self.turntableItems_) do
        local cfg = DfwDataMgr:getTurntableCfg(self.turntable_[i])
...
function DfwTurntableView:addTurntableItem()
...
function DfwTurntableView:nextTurnTable()
...
function DfwTurntableView:startTheDraw()
...
function DfwTurntableView:updateMethod()
...
function DfwTurntableView:continueTheDraw()
    local count = 1
...
function DfwTurntableView:endTheDraw()
...
function DfwTurntableView:registerEvents()
...
function DfwTurntableView:onLuckyWheelEvent(turnInfo, rewards)
    self.rewards_ = rewards
    self.targetIndex_ = turnInfo.turnIndex
    self:startTheDraw()
end

return DfwTurntableView
