local Anniversary2YearView = class("Anniversary2YearView", BaseLayer)

function Anniversary2YearView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.recharge.anniversary2YearView")
end

function Anniversary2YearView:initData()
...
function Anniversary2YearView:initUI(ui)
...
function Anniversary2YearView:addCountDownTimer()
    if not self.countDownTimer_ then
        self.countDownTimer_ = TFDirector:addTimer(1000, -1, nil, handler(self.onCountDownPer, self))
    end
end

function Anniversary2YearView:onCountDownPer()
...
function Anniversary2YearView:onShow()
    self.super.onShow(self)
end

function Anniversary2YearView:onHide()
    self.super.onHide(self)
end

function Anniversary2YearView:updateContentView()
    ...
    local giftData = RechargeDataMgr:getAnniversary2yearData()
...
function Anniversary2YearView:updateGiftItem(item, data)
    ...
    imgBg:setTexture(url.."012.png") -- 默认图片
    if data.extendData then
...
function Anniversary2YearView:updateCellItem(item, data, canTouch, posY)
    posY = posY or 25
...
function Anniversary2YearView:onGoodItemClickHandle(sender)
    if not sender.id then return end
    Utils:showInfo(sender.id, nil, true)
end


function Anniversary2YearView:registerEvents()
    self.super.registerEvents(self)
    self:registerCustomEvents()
end

function Anniversary2YearView:registerCustomEvents( )
    EventMgr:addEventListener(self,EV_RECHARGE_UPDATE,handler(self.updateContentView, self))
end

function Anniversary2YearView:removeCountDownTimer()
...
function Anniversary2YearView:removeEvents()
    self.super.removeEvents(self)
    self:removeCountDownTimer()
end

return Anniversary2YearView
