...
local LeagueOpenRedPacketView = class("LeagueOpenRedPacketView", BaseLayer)

function LeagueOpenRedPacketView:initData(redPacketData)
    self.redPacketData = redPacketData
end

function LeagueOpenRedPacketView:ctor(redPacketData)
    self.super.ctor(self)
    self:initData(redPacketData)
    self:showPopAnim(true)
    self:init("lua.uiconfig.league.leagueOpenRedPacketView")
end

function LeagueOpenRedPacketView:initUI(ui)
...
function LeagueOpenRedPacketView:refreshView()
    local packetCfg = LeagueDataMgr:getPacketCfgById(self.redPacketData.id)
...
function LeagueOpenRedPacketView:registerEvents()
    self.Button_check:onClick(function()
        AlertManager:closeLayer(self)
        Utils:openView("league.LeagueRedPacketDetailView", self.redPacketData)
    end)
end

return LeagueOpenRedPacketView
