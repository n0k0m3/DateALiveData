...
function SaveManager:saveUserInfoDemo(account,password,token,ip,port)
    CCUserDefault:sharedUserDefault():setStringForKey("account",account)
    ...
    CCUserDefault:sharedUserDefault():setStringForKey("port",port)
    CCUserDefault:sharedUserDefault():flush()
end

function SaveManager:getUserInfoDemo()
    local account = CCUserDefault:sharedUserDefault():getStringForKey("account");
    local password = CCUserDefault:sharedUserDefault():getStringForKey("password");
    return account,password;
end

function SaveManager:saveIsActivat(isActivat)
    CCUserDefault:sharedUserDefault():setBoolForKey("isActivat",isActivat)
    CCUserDefault:sharedUserDefault():flush()
end

function SaveManager:getIsActivat()
    return CCUserDefault:sharedUserDefault():getBoolForKey("isActivat");
end

function SaveManager:getServerInfoDemo()
    local t = {}

    local ip = CCUserDefault:sharedUserDefault():getStringForKey("ip");
    local port = CCUserDefault:sharedUserDefault():getStringForKey("port");


    t.address = ip..":"..port;--"192.168.0.191:8080"

    return t;
end

function SaveManager:getCurrentToken()
