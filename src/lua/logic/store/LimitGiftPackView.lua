...
local LimitGiftPackView = class("LimitGiftPackView", BaseLayer)

function LimitGiftPackView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.recharge.limitGiftPackView")
end

function LimitGiftPackView:initData()
...
function LimitGiftPackView:initUI(ui)
...
function LimitGiftPackView:onShow()
    self.super.onShow(self)
end

function LimitGiftPackView:onHide()
    self.super.onHide(self)
end

function LimitGiftPackView:updateContentView()
    ...
    local giftData = RechargeDataMgr:getLimitGiftData()
...
function LimitGiftPackView:updateGiftItem(item, data)
    ...
    if data.triggerEndDate and data.triggerEndDate > 0 then
        self.coolDown[Label_countdown] = data.triggerEndDate
    elseif data.startDate and data.endDate and serverTime >= data.startDate and serverTime < data.endDate then
...
function LimitGiftPackView:updateCellItem(item, data, canTouch, posY)
	posY = posY or 80
...
function LimitGiftPackView:onGoodItemClickHandle(sender)
	if not sender.id then return end
	Utils:showInfo(sender.id, nil, true)
end


function LimitGiftPackView:registerEvents()
    self.super.registerEvents(self)
    self:registerCustomEvents()
end

function LimitGiftPackView:registerCustomEvents( )
    EventMgr:addEventListener(self,EV_RECHARGE_UPDATE,handler(self.updateContentView, self))
    EventMgr:addEventListener(self,EV_LIMIT_GIFT_PUSH,handler(self.updateContentView, self))
end

function LimitGiftPackView:removeCountDownTimer()
...
function LimitGiftPackView:removeEvents()
    self.super.removeEvents(self)
    self:removeCountDownTimer()
end

return LimitGiftPackView
