...
function AddressView:initData(spreadQq)
    self.proviceName = ""
    self.municipalName = ""
    self.townName = ""
    self.cdTime = 60
    self.spreadQq = spreadQq or ""
end

function AddressView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.oneYear.addressView")
end
...
function AddressView:initUI(ui)
    ...
    self.Label_service_qq = TFDirector:getChildByPath(self.Panel_root, "Label_service_qq")
    self.Label_service_qq:setText("客服QQ："..self.spreadQq)
    ...
    Label_content:setTextById(63602)
...
function AddressView:updateAddress()
    ...
    self.fillInfo = clone(OneYearDataMgr:getFillInfo())
    ...
    local act = CCSequence:create({
        CCCallFunc:create(function()
            local nextTime = OneYearDataMgr:getNextTime()
...
function AddressView:updateProvinceList()
    ...
    for k,v in ipairs(proviceInfo) do
        ...
        Label_detail:setColor(ccc3(69,22,21))
        local ImageSelect = detailItem:getChildByName("Image_select"):hide()
        ...
        --detailItem:onTouch(function(event)
...
function AddressView:submitAddress()
    ...
    Utils:openView("oneYear.AddressConfirmView",self.newFillInfo)
