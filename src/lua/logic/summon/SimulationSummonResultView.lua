...
local SimulationSummonResultView = class("SimulationSummonResultView", BaseLayer)

function SimulationSummonResultView:initData(cid,reward, isShieldEffect)
...
function SimulationSummonResultView:ctor(...)
    self.super.ctor(self)
    self:initData(...)
    self:init("lua.uiconfig.summon.SimulationSummonResultView")
end

function SimulationSummonResultView:initUI(ui)
	...
	self.Button_ok = TFDirector:getChildByPath(ui, "Button_ok"):hide()
	
	
	self.Button_giveup = TFDirector:getChildByPath(self.Panel_result, "Button_giveup"):hide()
	self.Button_replace = TFDirector:getChildByPath(self.Panel_result, "Button_replace"):hide()
...
function SimulationSummonResultView:refreshView()
...
function SimulationSummonResultView:updateSpeed(index)
...
function SimulationSummonResultView:showEffect()
...
function SimulationSummonResultView:doComplete()
	...
	local simulationSummon =  SimulationSummonDataMgr:getSimulationSummon(self.cid_)
	if simulationSummon.simulateSummonCount + simulationSummon.sysSimulateSummonCount <= 3 then
		self.Label_continue:show()
		self.Panel_touch:show()
		self.Button_ok:show()
	else
		self.Button_giveup:show()
		self.Button_replace:show()
	end	
	--	self.cid_
	
	self.Label_continue:hide()

    for i, site in ipairs(self.rewardItem_) do
        local reward = self.reward_[i]
        local itemCfg = GoodsDataMgr:getItemCfg(reward.id)
        if itemCfg.superType == EC_ResourceType.HERO then
            if SimulationSummonDataMgr:isHaveHero(reward.id) then
...
function SimulationSummonResultView:turnOnReward(index)
...
function SimulationSummonResultView:showMoreResult()
...
function SimulationSummonResultView:updateResultItem(item, reward)
    local itemCfg = GoodsDataMgr:getItemCfg(reward.id)
...
function SimulationSummonResultView:playEffectAni(aniName, isLoop)
...
function SimulationSummonResultView:onRecvSimulationSummonReplace(data)
	--AlertManager:close()
	AlertManager:closeLayer(self)
end

function SimulationSummonResultView:onRecvSimulationSummonExchange(data)
	--AlertManager:close()
end


function SimulationSummonResultView:registerEvents()
	...
	self.Button_giveup:onClick(function()
		SimulationSummonDataMgr:reqSimulateSummonReplace(false,1)
	end)
	
	self.Button_replace:onClick(function()
		Utils:openView("summon.SimulationSummonExchangeView", self.cid_, 2, self.reward_)
	end)
...
function SimulationSummonResultView:onShow()
...
function SimulationSummonResultView:showResult()
...
function SimulationSummonResultView:isHighQuality(index)
    local reward = self.reward_[index]
    local itemCfg = GoodsDataMgr:getItemCfg(reward.id)
...
function SimulationSummonResultView:onCountDownPer(delta)
...
function SimulationSummonResultView:getIntervalSection(x)
...
function SimulationSummonResultView:removeEvents()
    self:removePowerTimer()
end

function SimulationSummonResultView:onTouchContinueEvent(event)
...
function SimulationSummonResultView:onClose()
    SimulationSummonDataMgr:resetAlreadyHaveHero()
end

return SimulationSummonResultView
