...
function KabalaAStarPath:getAroundTileds(tiledPos)
    ...
    for k,v in ipairs(aroundTileds) do        
        local Nil = KabalaTreeDataMgr:isNilTiled(v)
        if not Nil then
            local opacity = KabalaTreeDataMgr:getOpacity(v)
            if  opacity ~= 0 then
                local isObstacle = KabalaTreeDataMgr:isObstacle(v)
                if not isObstacle then
                    if self.desTiled.x == v.x and self.desTiled.y == v. y then
                        table.insert(around,v)
                    else                  
                        local kabalaItem = KabalaTreeDataMgr:getKabalaTreeItem(v)
    ...
    while next(self.openTable) do
        ...
        for k,v in ipairs(aroundTab) do
            ...
            if not self:isExitInTable(self.closeTable,v) then
                ...
                local valueG = curTiled.valueG + KabalaTreeDataMgr:calcValueDis(curTiled, v)
                local valueH = KabalaTreeDataMgr:calcValueDis(v,self.desTiled)
