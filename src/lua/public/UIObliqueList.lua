UIObliqueList = UIObliqueList or class("UIObliqueList", function()
...
function UIObliqueList:ctor(data)
    self:init(data);
end

function UIObliqueList:init(params)
    ...
    self.maxOffsetX             = params.offsetX * (self.numberInView - 1)/2 or 400;
...
function UIObliqueList:destroy()

end

function UIObliqueList:onTouchUp()
    self:jumpAction();
end

function UIObliqueList:moveCells(offsetY)
...
function UIObliqueList:isInCenter(posy)
...
function UIObliqueList:isInView(posy)
    return posy < self.triggerLen and posy + self.cellHeight > 0
end

function UIObliqueList:updatePositionByIndex(idx,offsetY)
    ...
    posx = (posy + cell:getContentSize().height*(0.5-cell:getAnchorPoint().y) - self.triggerLen / 2) / (self.triggerLen / 2) * self.maxOffsetX;

    --X翻转
    if self.isFlippingX then
        posx = - posx
    end
    posx = (self.size.width/2 - self.cellWidth/2 + self.cellWidth*cell:getAnchorPoint().x) + posx
...
function UIObliqueList:updateOne(cell,_type)
    local ischange = false
...
function UIObliqueList:appendCell(index)
    ...
    posx = (posy - self.triggerLen / 2) / (self.triggerLen / 2) * self.maxOffsetX;
...
function UIObliqueList:addCells()
...
function UIObliqueList:scrollTo(idx)
    self.selCell = self:getCellByIndex(idx);
    self:scrollToAction();
end

function UIObliqueList:jumpAction(isJumpTo)
...
function UIObliqueList:jumpTo(idx)
    self.__selIdx = idx
    self.selCell = self:getCellByIndex(idx);
...
function UIObliqueList:getCellCount()
    return #(self.elements);
end

function UIObliqueList:getSelCell()
    return self.selCell;
end

function UIObliqueList:getCellByIndex(idx)
    return self.elements[idx];
end

function UIObliqueList:getSelIndex()
    return self.selIndex;
end

function UIObliqueList:setDragEnabeled(isDrag)
    self.isDrag = isDrag;
end

function UIObliqueList:isDragEnabeled()
    return self.isDrag;
end