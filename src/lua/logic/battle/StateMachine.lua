...
function StateMachine:canDoEvent(eventName)
    -- print("self.inTransition_",self.inTransition_)
    -- print("self.map_[eventName][self.current_]",self.map_[eventName][self.current_])
    -- print("self.map_[eventName][StateMachine.WILDCARD]",self.map_[eventName][StateMachine.WILDCARD])
...
function StateMachine:onError_(event, error, message)
    printError(string.format("%s [StateMachine] ERROR: error %s, event %s, message %s ", tostring(self.target_), tostring(error), event.name, message))
end
